...
def main():
    ...
    for V_flow_borehole in V_flow_rates:
        ...
        double_u_tube_parallel = plat.borehole_heat_exchangers.MultipleUTube(
            m_flow_borehole, fluid, borehole, pipe_d, grout, soil,
            config='parallel')
    ...
    for i, tube in enumerate(borehole_values):
        ax_1[i].scatter(borehole_values[tube]['Re'],
                   borehole_values[tube]['Rb'], label=tube + ' (GLHEDT)')
        ax_1[i].scatter(borehole_validation_values[tube]['Re'],
                   borehole_validation_values[tube]['Rb'],
                   label=tube + ' (GLHEPRO)', marker='x')
        ax_1[i].grid()
        ax_1[i].set_axisbelow(True)
        ax_1[i].legend()

        ax_2[i].scatter(borehole_values[tube]['V_dot'],
                     borehole_values[tube]['Rb'], label=tube + ' (PLAT)')
        ax_2[i].scatter(borehole_validation_values[tube]['V_dot'],
                     borehole_validation_values[tube]['Rb'],
                     label=tube + ' (GLHEPRO)', marker='x')
        ax_2[i].grid()
